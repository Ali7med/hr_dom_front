...
- قرارات/ملاحظات: **مكان الصفحة = تبويب إعدادات** (بدل `/backups` المسطّح الذي في ملف التوجيه) — مواءمةً لإعادة تصميم التنقّل FE-NAV التي نقلت كل صفحات الإدارة تحت `/settings/*`؛ التبويب يُفلتَر بالصلاحية فيظهر لـ Super Admin فقط. بنية سجلّ النسخة **دفاعية** (الحجم بايتات أو نصّ جاهز؛ الحالة قيم معروفة + fallback). **سؤال معلّق للمدير:** اللوحة صارت تذكر «تشفير AES بكلمة مرور» في BE-64/65 لكن العقد (`BackupSettingsInput`) بلا حقل كلمة مرور — إن كانت تُدار من اللوحة فيلزم تحديث العقد (Contract-First) ثم إضافة حقل لـ FE-21 (التنبيه الأمني صِيغ محايداً تجاه التشفير). **التحقّق الحيّ محجوب كلياً على BE-64/BE-65**: المسارات 404، وصلاحية `backups.manage` + مستخدم Super Admin غير متوفّرَين للتجربة.
